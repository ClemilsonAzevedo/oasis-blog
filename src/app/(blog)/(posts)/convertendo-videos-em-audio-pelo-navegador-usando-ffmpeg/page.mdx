...
*Se você tiver interesse no código dessa função, me contate no [Twitter](https://x.com/dieegosf)!*
...
<Image className="rounded-lg" src={audioConversionScreenshot} alt="Captura de tela da plataforma Nivo que destaca a barra de progresso de conversão de vídeo em áudio" width={768} height={290} />
...
await ffmpeg.exec([ // [!code highlight:10]
  '-i',
  inputFile.name,
  '-vn',
  '-b:a',
...
const audioFile = new File([audioFileBlob], `${outputId}.mp3`, { // [!code highlight:3]
  type: 'audio/mpeg',
})
